...
export default class TwentytwoWrapper extends LightningElement {
	...
	setNav(string) {
		...
		switch(string) {
			...
			case 'challenge':
				...
				this.headerTitle = 'Daily Workout';
				break;
			default:
				this.showLeaderboardPage = true;
				this.headerTitle = 'Leaderboard';
		...
		this.template.querySelector(`.${string}`).checked = true;
	}
    ...
    handleWorkoutClick() {
        this.setNav('workouts');
	}

    handleMonthlyClick(){
        this.setNav('challenge');
	}
	...
	buildAllChallengeEntries(incomingChallenges, incomingEntries) {
        ...
        incomingChallenges.forEach(challenge => {
			...
			rowData.Title__c = challenge.Title__c;
